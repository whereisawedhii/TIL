...
def create_user(old_user_list) :
    False_num = 0
    user_list = []
    for each in old_user_list :
        validate_result = is_validation(each['blood_group'], each['company'], each['mail'], each['name'], each['website'])
        if validate_result == True : 
            user_list.append(each)
        elif validate_result == 'blocked':
            False_num += 1
        elif validate_result != True and validate_result != 'blocked' : 
            False_num += 1
            validate_bool, failed_list = validate_result
            for any in failed_list : 
                each[any] = None
            user_list.append(each)
...
def is_validation(blood, company, mail, name, website) :
    valdation_failed = []
    if blood_types.count(blood) == 0 :
        valdation_failed.append('blood_group')
    if black_list.count(company) != 0 :
        return 'blocked' 
        exit()

    if mail.find('@') == -1 :
        valdation_failed.append('mail')
    if len(name) < 2 or len(name) > 30 :
        valdation_failed.append('name')
    if len(website) == 0 : 
        valdation_failed.append('website')

    if len(valdation_failed) == 1 :
        return (False,valdation_failed)
    elif len(valdation_failed) >= 2 :
        return (False,valdation_failed)
    elif len(valdation_failed) == 0 :
        return True
